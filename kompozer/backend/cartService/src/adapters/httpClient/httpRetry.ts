 ...
 * service's outbound HTTP adapters (calls to catalogService/orderService).
 *
 ...
 *   Set to false for non-idempotent writes where a timed-out request may
 *   already have been applied server-side (e.g. order submission).
 * - 5xx responses — only when `retryOn5xx` is true (default true), same
 *   caveat as timeout.
 *
 * Never retries 4xx responses (client error, retrying won't help) or a
 * malformed JSON body (the server completed the request; retrying a
 * non-idempotent write here would risk a duplicate side effect for no
 * benefit — the response will be malformed again).
 */
