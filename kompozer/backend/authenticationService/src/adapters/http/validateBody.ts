 ...
 * On failure, raises the existing ValidationError (same shape already
 * produced by use-case-level validation) so errorMiddleware needs no changes.
 * On success, req.body is replaced with the parsed/stripped output —
 * unknown fields are dropped, preventing mass-assignment style payloads.
 */
