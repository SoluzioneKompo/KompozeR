...
\item proxy REST verso \texttt{/auth}, \texttt{/catalog}, \texttt{/cad}, \texttt{/cart}, \texttt{/orders}, \texttt{/payments}, \texttt{/notifications}, \texttt{/chatbot} e \texttt{/reports}.
\end{itemize}
...
Servizio ancora allo stadio di scheletro architetturale: i provider PayPal e carta sono adapter stub. I path reali sono:

\begin{itemize}
\item \texttt{GET /payments/health};
\item \texttt{POST /payments};
\item \texttt{GET /payments/order/:orderId};
\item \texttt{GET /payments/:paymentId};
\item \texttt{POST /payments/:paymentId/confirm}.
\end{itemize}

L'ultimo endpoint simula, per ora, la callback che in futuro arrivera' dal provider reale.
