...
I database sono separati per servizio. Nei compose file di sviluppo e di esecuzione sono presenti istanze MongoDB dedicate a auth, catalog, cad, cart, notification, chatbot, order, payment e reporting, oltre a Redis per catalogo e notifiche.
...
\subsection{paymentService}
Servizio dedicato al pagamento, introdotto come scheletro architetturale: espone creazione, lettura e conferma di un tentativo di pagamento legato a un ordine (\texttt{orderId}), senza modificare orderService, che resta disaccoppiato. La logica applicativa segue lo stesso pattern hexagonal degli altri servizi (domain/ports, use case, adapter HTTP e persistence) e delega l'interazione con il provider a un port \texttt{PaymentGateway}, con due adapter stub (\texttt{PayPalGateway}, \texttt{CardGateway}) pronti per l'integrazione reale ma che oggi restituiscono subito uno stato \texttt{PENDING} con un riferimento fittizio. Un endpoint di conferma (\texttt{POST /payments/:paymentId/confirm}) simula l'esito che in futuro arriverebbe dal provider (webhook/callback), cosi' l'intero flusso di checkout resta testabile end-to-end senza credenziali reali.
...
\item dopo il checkout il frontend avvia un pagamento su payment service passando l'\texttt{orderId} dell'ordine appena creato; il collegamento e' solo per riferimento, nessun servizio esistente viene modificato;
