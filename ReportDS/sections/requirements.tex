...
implemented. Requirements are elicited from the stakeholders (customers,
...
\begin{description}
  ...
  \item[Product family] one of the three mutually incompatible catalogs
  (Tondo, Quadro, Qube); components of different families cannot be combined.
  ...
  \item[Subscription] a user's interest in a specific SKU, used to notify the
  user when that SKU changes price or availability.
...
Several non-functional requirements are inherently distributed-system concerns;
they are highlighted and later mapped to design choices in \cref{design}.
...
These constraints bound the realisation of the system. They are motivated by
the academic context of the project (Distributed Systems and ASW courses),
which asks for a genuinely distributed, containerised system, and by the reuse
of a common technology baseline across the team.
...
\begin{description}
  ...
  Node.js/TypeScript with Express, and the frontend as a Vue~3 SPA, to share a
  single toolchain and skill set within the team.
...
\begin{itemize}
  ...
  several contexts (configurator, cart, notifications, chatbot, reporting). Much
...
\begin{itemize}
  ...
  gateway can scale horizontally. Extreme, global-scale elasticity is out of
  scope.
  ...
  is primary. Full failure transparency is only pursued for the core workflow.
  ...
  matter (NFR2, NFR8), and many sessions run concurrently; hard real-time or
  high-throughput bandwidth optimisation is not a goal.
\end{itemize}
