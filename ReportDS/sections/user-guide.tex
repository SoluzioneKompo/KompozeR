...
roles --- \texttt{GUEST}, \texttt{BASE} and \texttt{ADMIN}
(\cref{requirements}) --- following the navigation structure of the
frontend. The interface itself is in Italian; UI labels are quoted verbatim
below with an English gloss. Screenshots are not included in this draft;
each screen is instead marked with a placeholder comment
(\texttt{\% SCREENSHOT: ...}) in the \LaTeX{} source, ready to be replaced
...
% SCREENSHOT: AuthView — login/register card with the guest button.
...
% SCREENSHOT: CadView — the five-step wizard with the BOM/preview panel.
...
\begin{enumerate}
  ...
  product families --- Tondo, Quadro, Kube, Intelligente
  (\cref{requirements}) --- then ``Salva categoria''. Changing this later
  discards any column plan or design already produced, with a two-step
  confirmation dialog, matching the reset rule of \cref{fig:state-configuration}.
  ...
  % SCREENSHOT: CadView step 4 — level-by-level design with the reason codes.
...
session (\cref{modelling}): a short, unambiguous code (e.g.\
\texttt{A3KP7X}) is shown in a dialog with a ``Copia codice'' (copy) button.
% SCREENSHOT: CadView — the "Sessione collaborativa attiva" dialog with the session code.
...
(\cref{interaction,behaviour}). A status line (``Collaborazione attiva ·
...
% SCREENSHOT: two browser windows showing the same configuration in sync.
...
and, for each of the five most recent, an ``Apri in CAD'' button to resume
editing (FR6, FR7) and, once \texttt{FINALIZED}, a ``Riordina'' (reorder)
button that adds its BOM straight to the cart again.
% SCREENSHOT: ConfigurationsView — the list of saved configurations with KPI cards.
...
catalog can no longer provide (\cref{interaction}).
% SCREENSHOT: CartView — line items and the checkout summary panel.
...
finalises the order.
% SCREENSHOT: the shipping-details checkout modal.

\subsection{Notifications}\label{ug-notifications}

Authenticated, non-guest users see a bell icon with an unread-count badge in
...
unread-only and a button to mark one as read; new notifications additionally
appear as a temporary toast in the corner of the screen while the user is
active.
% SCREENSHOT: NotificationsView — the notification list, and a toast popup.

\subsection{Chatbot}\label{ug-chatbot}

The chatbot (route \texttt{/chatbot}) is a dedicated chat screen (FR5),
optionally opened from within a configuration to answer questions in that
context. Messages are exchanged with a composer at the bottom (``Invia'' to
send); a typing indicator shows while the bot is composing an answer, which
can concern, for instance, the current price of a specific component by SKU.
% SCREENSHOT: ChatbotView — a conversation with the typing indicator.
...
\begin{itemize}
  ...
  actions immediately trigger the catalog-change propagation of
  \cref{fig:seq-catalog-event}.
  % SCREENSHOT: AdminCatalogView — the component list and the "add component" modal.
  ...
  % SCREENSHOT: AdminOrdersView — the order list with status filters.
  ...
  % SCREENSHOT: AdminReportsView — the KPI cards and the per-day chart.
\end{itemize}
