...
that extends a classic e-commerce scenario with a domain-specific
...
components. The catalog is organised in four product
categories (Tondo, Quadro, Intelligente and Kube), and the configurator enforces the domain
...
\begin{itemize}
  \item \textbf{What} the software does: it lets users build a valid shelving configuration while seeing an
  up-to-date preview and price, automatically save configurations to their profile, add them
  ...
  the chatbot, and the collaborative CAD sessions, where several users edit the
  same configuration and see each other's changes in real time.

  \item \textbf{Data to store}: the system persists users and sessions,
...
\begin{itemize}
  \item \textbf{Resource sharing}: a single update of the catalog and its
  ...
  configuration, cart, orders, notifications, chatbot, reporting). Designing
  each of them as an independently deployable service allows them to evolve, scale, and
...
The deployment is logically centralised, and the interesting
